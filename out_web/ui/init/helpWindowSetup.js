...
function fillFilterContent(engine) {
    ...
    try {
        // 1. Llamada al puente de C++
        const rawJson = engine.ccall('get_filter_registry_json', 'string', [], []);
        const registry = JSON.parse(rawJson);

        container.innerHTML = ''; // Limpiamos el placeholder

        // 2. Iteración del mapa (name es la key, info es el descriptor)
        Object.entries(registry).forEach(([name, info]) => {
            const card = document.createElement('div');
            card.className = "group border border-zinc-800 bg-zinc-900/20 p-4 rounded-xl hover:border-zinc-700 transition-all";
            
            // Mapeamos los parámetros del vector<string>
            const paramsList = info.params.length > 0 
